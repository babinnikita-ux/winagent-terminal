...
import { AgentPreset, SplitNode, ThemeConfig } from '../../shared/types';
    ...
    if (surfaceId && window.wmux.pty.has) {
      window.wmux.pty.has(surfaceId).then((exists: boolean) => {
        if (exists) {
          ...
          window.wmux.pty.create({ shell: effectiveShell, cwd: cwd ?? '', env: {}, surfaceId, startupCommands: startupCommandsRef.current, agentPreset, resumeAgentSession, cols: initialCols, rows: initialRows })
      ...
      window.wmux.pty.create({ shell: effectiveShell, cwd: cwd ?? '', env: {}, startupCommands: startupCommandsRef.current, agentPreset, resumeAgentSession, cols: initialCols, rows: initialRows })
        .then((created: { id: string; shell: string; startupCommandsConsumed?: boolean }) => {
          if (disposed) return;
          setResolvedShellForSurface(surfaceId, created.shell);
          attachToPty(created.id);
